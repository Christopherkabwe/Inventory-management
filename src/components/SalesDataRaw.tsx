...
export default function SalesDataPage() {
    ...
    const fetchSales = async () => {
        try {
            ...
            setSales(data || []); // Update this line
            setFilteredSales(data || []); // Update this line
        } catch (err) {
            console.error(err);
            setError(err);
        } finally {
        ...
        const filtered = sales
            .filter((s) => {
                ...
                const matchesStartDate =
                    !startDate || saleTime >= new Date(`${startDate}T00:00:00`).getTime();

                const matchesEndDate =
                    !endDate || saleTime <= new Date(`${endDate}T23:59:59`).getTime();
