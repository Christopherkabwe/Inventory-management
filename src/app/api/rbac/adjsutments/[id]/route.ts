...
export async function PUT(req: Request, { params }: Params) {
    ...
    try {
        ...
        const { locationId, type, reason, items }: { locationId: string; type: "DAMAGED" | "EXPIRED" | "MANUAL"; reason?: string; items: { productId: string; quantity: number }[]; } = await req.json();
        ...
        const updatedAdjustment = await prisma.$transaction(async (tx) => {
            ...
            for (const productId of allProductIds) {
                ...
                switch (type) {
                    ...
                    case "EXPIRED":
                        ...
                        await updateInventoryHistory(productId, locationId, -delta, new Date());
                        break;
                    case "MANUAL":
                        data.increment = delta > 0 ? delta : 0;
                        data.decrement = delta < 0 ? -delta : 0;
            ...
            return tx.adjustment.update({ where: { id }, data: { locationId, type, reason, updatedAt: new Date() }, include: { items: true }, });
        });
            ...
            for (const item of adjustment.items) {
                ...
                switch (adjustment.type) {
                    ...
                    case "EXPIRED":
                        ...
                        await updateInventoryHistory(item.productId, adjustment.locationId, item.quantity, new Date());
                        break;
                    case "MANUAL":
