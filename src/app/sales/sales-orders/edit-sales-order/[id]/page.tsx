...
const EditSalesOrderPage: React.FC = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                // 1️⃣ Fetch options and order data in parallel
                const [cRes, pRes, lRes, orderRes] = await Promise.all([
                    fetch("/api/customers").then(r => r.json()),
                    fetch("/api/options/products").then(r => r.json()),
                    fetch("/api/locations").then(r => r.json()),
                    fetch(`/api/order-flow/orders/${orderId}`).then(r => r.json()),
                ]);

                setCustomers(cRes.customers ?? []);
                setProducts(pRes ?? pRes ?? []);
                setLocations(lRes.locations ?? []);

                // 2️⃣ Populate order data
                ...
                // 3️⃣ Fetch stock quantities for each product at the selected location
                if (order.locationId && order.items.length > 0) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/order-flow/orders/${orderId}`, {
                method: "PUT",
            ...
            router.push(`/sales/invoices/create-invoice/${orderId}`);
        } catch (err: any) {
                                ...
                                <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                                    ...
                                    <div className="grid grid-cols-1">
                                        <NumberInput
                                            value={item.quantity}
                                            min={1}
                                            max={availableQuantities[item.productId] ?? Infinity} // ✅ limit by available stock
                                            required
                                            onChange={(value) => {
                                                const max = availableQuantities[item.productId] ?? Infinity;
                                                const copy = [...items];
                                                copy[index].quantity = Math.min(value, max); // ✅ ensure it never exceeds stock
                                                setItems(copy);
                                            }}
                                        />
                                        {/* Show available stock below the input */}

                                        {availableQuantities[item.productId] !== undefined && (
                                            <span className="text-xs text-gray-500 px-5 py-1">
